...
alias notes='cd ~/Documents/Obsidian/ && vim'
alias notesup='TEMP_DIR=$(pwd) cd ~/Documents/Obsidian/ && \
  git add . > /dev/null && \
  git status && \
  git commit -m "chore: update notes" > /dev/null && \
  git push > /dev/null; \
  echo $TEMP_DIR | cd'
alias notesdown='cd ~/Documents/Obsidian/ && git pull'
